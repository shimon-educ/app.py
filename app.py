...
if input_func:
    ...
    try:
            ...
            if user_asymp and user_horiz:
                try:
                    ...
                    if correct_v and correct_h:
                        st.success(f"מצוין! מצאת את כל האסימפטוטות.")
                        show_plot = True
                    else:
                        if not correct_v: st.error("יש טעות באסימפטוטות האנכיות.")
                        if not correct_h: st.error("יש טעות באסימפטוטה האופקית.")
                except:
                    st.warning("נא להזין מספרים תקינים.")

            if st.button("הצג פתרון וסרטט"):
                st.info(f"אנכיות: x = {true_pts_str}, אופקית: y = {format_num(true_horiz)}")
                show_plot = True
        ...
        if show_plot:
            ...
            fig.add_hline(y=h_val, line_dash="dash", line_color="blue",
                          annotation_text=f"y={format_num(h_val)}", annotation_position="right")
